...
function reorder_pad_pieces() {
		...
		for (var row = 1; row < 9; row++) {
			...
			for (var column = 1; column < 9; column++) {
				...
				temporary_piece_reference.attr({
					...
					'data-co-x': row * (pads + 1),
					'data-co-y': column * (pads + 1)
				});
			}
		}
	}
	...
	for (var i = 0; i < ssp_length; i++) {
		...
		var active_piece_co_x = '';
		var piece_count = 0;

		$('.pad_piece[data-co-y="' + (i + 1) + '-"][data-state="active"]').each(function () {
			active_piece_co_y = $(this).attr('data-co-y');

			var selected_select = $('.selects .select').eq(active_piece_co_y - 1);
			var selected_select_class = selected_select.attr('class').split(" ")[0];
			var class_trim = selected_select_class.substring(0, selected_select_class.length - 7).replace(/_/g, '-');
	...
	for (var i = 0; i < $('body').find('.duplicate_pad').length; i++) {
		...
		if (i > 0) {
			...
			$('.duplicate_pad').eq(i - 1).parents('.pad:eq(0)').append(sequence_flow_arrow);
